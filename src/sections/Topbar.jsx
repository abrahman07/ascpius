...
import { Bell, CaretDown, Globe, Check } from "@phosphor-icons/react";
...
export default function Topbar() {
  ...
  return (
    <section className="bg-topbar w-full py-3 px-6 flex items-center justify-between border-b border-foreground/10">
      {/* Logo */}
      <div className="flex items-center gap-3">
        <OmniMedLogo />
        <span className="omni-header__brand-name text-white text-xl">{t.brandName}</span>
      </div>
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <span className="text-[13px] font-medium text-white">{t.langLabel}</span>
            <CaretDown 
              size={14} 
              weight="regular" 
            ...
            >
              {LANGUAGES.map((lang) => (
                <li key={lang.code} role="option" aria-selected={currentLang === lang.code}>
                  <button
                    type="button"
                    className={`w-full text-left px-4 py-2 flex items-center justify-between hover:bg-gray-100 transition-colors ${
                      currentLang === lang.code ? 'bg-gray-50 text-primary font-medium' : 'text-gray-700'
                    }`}
        ...
        >
          <img
            src="https://c.animaapp.com/mpzcou9j6aQXGf/img/user-avatar.png"
            alt="User profile avatar"
            className="w-8 h-8 rounded-full object-cover"
          />
          <CaretDown size={14} weight="regular" className="text-white/60" />
        </button>
      </div>
    </section>
  );
